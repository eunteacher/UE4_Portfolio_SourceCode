#include "BehaviorTree/CBTTaskNode_SummonAction.h"
#include "Global.h"
...
UCBTTaskNode_SummonAction::UCBTTaskNode_SummonAction()
{
	NodeName = "SummonAction";

	bNotifyTick = true;
}

EBTNodeResult::Type UCBTTaskNode_SummonAction::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (state->IsDead() == true || state->IsFlying() == true || state->IsSummonAction() == true)
	{
		return EBTNodeResult::Failed;
	}
...
void UCBTTaskNode_SummonAction::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
