#include "BehaviorTree/CBTTaskNode_SpecialAction.h"
#include "Global.h"
...
UCBTTaskNode_SpecialAction::UCBTTaskNode_SpecialAction()
{
	NodeName = "SpecialAction";

	bNotifyTick = true;
}

EBTNodeResult::Type UCBTTaskNode_SpecialAction::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (state->IsDead() == true || state->IsFlying() == true || state->IsSpecialAction() == true)
	{
		return EBTNodeResult::Failed;
	}
...
void UCBTTaskNode_SpecialAction::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
